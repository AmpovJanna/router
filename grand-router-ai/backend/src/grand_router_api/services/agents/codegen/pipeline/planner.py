...
def run_plan(
    *,
    ...
    files_to_touch = [str(x).strip() for x in (data.get("files_to_touch") or []) if str(x).strip()]
    approach = str(data.get("approach") or "").strip() or "Implement the minimal correct change."
