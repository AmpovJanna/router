...
from ..pipeline.utils import read_prompt, safe_truncate
...
def run_debugger(*, task: str, context: dict[str, Any], patch: str | None = None) -> DebugResult:
    ...
    try:
        ...
        if isinstance(obj, dict):
            ...
            likely = [str(x).strip() for x in (obj.get("likely_root_causes") or []) if str(x).strip()]
            proposed = str(obj.get("proposed_fix") or "").strip()
            return DebugResult(reasoning=reasoning, likely_root_causes=likely[:8], proposed_fix=proposed)
    except Exception:
        pass

    return DebugResult(reasoning=(raw or "").strip(), likely_root_causes=[], proposed_fix="")
